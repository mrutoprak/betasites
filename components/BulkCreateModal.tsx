...
import { X, Play, Loader2, CheckCircle2, AlertCircle, Clock, Layers, Folder as FolderIcon, Square, ArrowRightLeft, RefreshCw } from 'lucide-react';
...
export const BulkCreateModal: React.FC<BulkCreateModalProps> = ({
  ...
  // Ref for mode to access latest value in async loop without dependency issues
  const modeRef = useRef<'turkish_to_arabic' | 'arabic_to_turkish'>('turkish_to_arabic');

  // Update modeRef when mode state changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setMode('turkish_to_arabic'); // Reset default
    }
  ...
  useEffect(() => {
    ...
    const processCurrent = async () => {
      ...
      // Use ref to get the mode that was selected when processing started/resumed
      const currentMode = modeRef.current;

      try {
        // 1. Generate Text
        setCurrentStep('Analyizing & Writing Story...');
        const data = await generateMnemonicData(word, currentMode, textModel);
  ...
  const hasFailures = status === 'done' && failedLogs.length > 0;

  const handleRetryFailed = () => {
    const wordsToRetry = failedLogs.map(l => l.word);
    setQueue(wordsToRetry);
    setCurrentIndex(0);
    setLogs([]);
    setStatus('generating');
    processingRef.current = true;
          ...
          {status === 'idle' ? (
            ...
            <div className="p-4 flex flex-col h-full gap-4">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200/50 flex flex-col flex-grow">
                ...
                <textarea 
                  className={`flex-grow w-full resize-none outline-none text-[16px] text-gray-900 placeholder:text-gray-300 font-mono leading-relaxed ${mode === 'arabic_to_turkish' ? 'text-right' : ''}`}
                  placeholder={mode === 'turkish_to_arabic' ? "Araba\nKitap\nKalem" : "سيارة\nكتاب\nقلم"}
                  value={inputText}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="bg-white p-6 border-b border-gray-100 flex flex-col items-center justify-center gap-3">
                ...
                {status === 'done' ? (
                   <div className={`flex flex-col items-center animate-in zoom-in ${hasFailures ? 'text-orange-500' : 'text-green-500'}`}>
                      {hasFailures ? <AlertCircle size={48} className="mb-2" /> : <CheckCircle2 size={48} className="mb-2" />}
                      <span className="text-xl font-bold text-gray-900">
                        {hasFailures ? `Completed with ${failedLogs.length} errors` : 'Completed Successfully'}
                      </span>
                      {hasFailures && <p className="text-sm text-gray-500 mt-1">Do you want to retry the failed items?</p>}
                   </div>
                ) : (
              ...
              <div className="p-4 bg-white border-t border-gray-200">
                 {status === 'done' ? (
                   <div className="flex gap-2 w-full">
                     {hasFailures && (
                        <NebulaButton onClick={handleRetryFailed} className="flex-1 flex items-center justify-center gap-2">
                            <RefreshCw size={18} /> Retry Failed ({failedLogs.length})
                        </NebulaButton>
                     )}
                     <NebulaButton onClick={handleClose} className={`flex-1 ${hasFailures ? 'bg-gray-200 text-gray-800 hover:bg-gray-300' : ''}`}>
