...
import { Card, Folder, AppMode } from '../types';
...
export const CreateModal: React.FC<CreateModalProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const genMode = appMode === 'medical' ? 'medical' : mode;
      const data = await generateMnemonicData(inputWord, genMode, textModel);
          ...
          {isLoading ? (
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-200/50">
                ...
                <div className="p-4 flex items-center gap-3">
                  <span className="text-[17px] text-gray-900 w-16 font-normal shrink-0">
                      {appMode === 'medical' ? 'Terim' : (mode === 'turkish_to_arabic' ? 'Meaning' : 'Word')}
                  </span>
                  <input 
                    type="text" 
                    value={inputWord} 
                    onChange={(e) => setInputWord(e.target.value)} 
                    placeholder={appMode === 'medical' ? 'Ör: Abdomen, Absorpsiyon...' : (mode === 'turkish_to_arabic' ? 'Enter Turkish meaning...' : 'Enter Arabic word...')}
                    className={`flex-1 text-[17px] text-gray-900 outline-none bg-transparent ${appMode === 'arabic' && mode === 'arabic_to_turkish' ? 'font-amiri text-right' : ''}`}
                    dir={appMode === 'arabic' && mode === 'arabic_to_turkish' ? 'rtl' : 'ltr'}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200/60 overflow-hidden divide-y divide-gray-100/80">
                ...
                <label className="w-20 text-[15px] text-gray-500 shrink-0">{appMode === 'medical' ? 'Açıklama' : 'Meaning'}</label>
                   <input 
                    type="text" 
                ...
                <div className="flex items-center p-3">
                  <label className="w-20 text-[15px] text-gray-500 shrink-0">{appMode === 'medical' ? 'Terim' : 'Arabic'}</label>
                  <div className="flex-1 flex gap-2 items-center">
                    <input 
                        type="text" 
                        value={arabicWord} 
                        onChange={e => setArabicWord(e.target.value)} 
                        className={`flex-1 text-[20px] outline-none bg-transparent text-left ${appMode === 'arabic' ? 'font-amiri' : ''}`}
                        dir={appMode === 'arabic' ? 'rtl' : 'ltr'}
                        placeholder={appMode === 'medical' ? 'Medical term' : 'Arabic word'}
                    />
