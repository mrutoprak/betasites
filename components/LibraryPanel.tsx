...
import { Plus, Trash2, Settings, Folder as FolderIcon, X, Layers, Download, ChevronDown, AlignJustify, Grip } from 'lucide-react';
...
const FolderGroup: React.FC<{
  ...
  onDelete?: () => void;
}> = ({ title, count, isOpen = false, children, onDelete }) => {
...
export const LibraryPanel: React.FC<LibraryPanelProps> = ({
  ...
  // View Mode: 'tabs' (horizontal chips) or 'tree' (vertical folders)
                        ...
                        >
                            {folderCards.length > 0 ? folderCards.map(card => (
                                <CardListItem
                                    ...
                                    onClick={handleCardClick}
                                    onAction={onActivateCard}
                                    // Selection disabled in tree view for simplicity, or we can enable it later
