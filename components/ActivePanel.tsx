...
import { startAlarm, stopAlarm, playNotificationSound } from '../utils/audio';
...
export const ActivePanel: React.FC<ActivePanelProps> = ({ 
  ...
  // isAlarmActiveRef is not needed for loop anymore but can track if we already played sound for this 'tick' if needed.
  // Actually, we play sound once per timer fire. Timer fires when item is due.
  // If multiple items are due, we might fire multiple times if logic loops.
  // But scheduleSmartTimer is recursive only via setTimeout.
  // Let's assume fire-and-forget is fine.
  ...
  useEffect(() => {
    ...
    // No stopAlarm on unmount needed for one-shot sound
  }, []);
  ...
  // Removed user interaction handler for stopping alarm since alarm stops itself
  /*
  useEffect(() => {
