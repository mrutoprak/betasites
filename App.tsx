...
import { Card, Folder, SRS_INTERVALS, AppMode } from './types';
import { BookOpen, Clock, Stethoscope } from 'lucide-react';
...
export default function App() {
  ...
  // App Mode: 'arabic' or 'medical'
  const [appMode, setAppMode] = useState<AppMode>(() =>
    (localStorage.getItem('app-mode') as AppMode) || 'arabic'
  );
  ...
  // Filter cards based on current app mode
  const filteredCards = useMemo(() => 
    cards.filter(c => (c.cardType || 'arabic') === appMode),
    [cards, appMode]
  );
    ...
    <>
      ...
      <div className="flex flex-col h-full w-full overflow-hidden relative text-gray-900">
        ...
        <nav className="bg-[#F9F9F9] border-t border-gray-200 pb-[calc(16px+env(safe-area-inset-bottom))] pt-2 flex-none flex items-stretch z-20 shadow-[0_-1px_0_rgba(0,0,0,0.05)] h-auto min-h-[80px]">
          ...
          {/* Mode Switcher */}
          <button 
            onClick={() => setAppMode(prev => prev === 'arabic' ? 'medical' : 'arabic')}
            className={`flex-1 flex flex-col items-center justify-center gap-1 transition-colors duration-200 py-1 ${
              appMode === 'medical' ? 'text-emerald-600' : 'text-gray-400'
            }`}
          >
            <Stethoscope size={24} strokeWidth={appMode === 'medical' ? 2.5 : 2} />
            <span className="text-[10px] font-medium">{appMode === 'medical' ? 'Tıp Modu' : 'Tıp Modu'}</span>
          </button>
        </nav>
